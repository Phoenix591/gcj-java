...
inherit check-reqs gnome2-utils java-pkg-2 java-vm-2 multiprocessing pax-utils prefix versionator virtualx flag-o-matic
...
src_prepare() {
	append-cflags $(test-flags-CC -Wnostringop-truncation)
	append-cxxflags $(test-flags-CXX -Wnostringop-truncation)
	default
}
src_configure() {
...
src_compile() {
	#Broken old kernel version check, assuming everyone's past 2.x by now
...
src_install() {
	...
	java-vm_install-env "${FILESDIR}/icedtea.7.env.sh"
